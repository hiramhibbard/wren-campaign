...
- `dungeon-magazine-side-trek` or short scenario;
- `boxed-set-scenario`;
- `sourcebook-embedded-adventure-site`;
- `sourcebook-embedded-scenario-seed`;
- `published-encounter-or-mini-adventure`;
- other explicitly available AD&D 2e-compatible scenario sources.
...
Before seeding a published scenario, evaluate only the dimensions that matter:
- active setting compatibility;
- region/geography/environment compatibility;
- settlement/site type;
- level/risk range as a **world danger**, not a promise of balance;
- tone and old-school AD&D 2e fit;
- required races/cultures/deities/cosmology/factions;
- assumptions about magic, technology, politics, law, economy, or travel;
- amount of adaptation required;
- conflicts with established geography/history/world state;
- duplication of existing campaign elements;
...
### Candidate
The DM has identified a potentially useful published scenario. It is not campaign canon and creates no world facts.

### Prepared Possibility
The scenario has been reviewed enough that it could plausibly occupy an unresolved part of the world. It still does not create established geography, NPCs, clues, or history merely by being prepared.

### Seeded
The minimum necessary parts have entered campaign causality: for example a location, rumor, NPC activity, aftermath, visible sign, faction interest, or established historical fact. Seed only what is actually needed.

### Active
Wren has materially interacted with the scenario, or its actors/processes now independently affect active world state.

### Dormant / bypassed / resolved
Preserve continuity. A seeded or active scenario is not reset because Wren leaves, ignores it, or approaches from an unexpected direction.
...
Do not search every Dungeon issue every turn. Use targeted retrieval based on environment, setting, scenario type, and active world need.
...
- sites preserve changes;
- alarms spread;
- victims may die or escape;
- antagonists may relocate;
- treasure may move;
