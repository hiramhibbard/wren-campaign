...
Fetch and obey CAMPAIGN_BOOTSTRAP.md and STATE_SCHEMA.md.

Fetch this root manifest and state/INDEX.md.

Load the Current Resume Working Set below.

List checkpoints/ and apply every real campaign checkpoint with sequence greater than checkpoint_baseline in strict numerical order.

Verify the Current Resume Packet against Wren's character/resources, chronology/location, active threads, relevant NPC state, and DM-only state.

If any conflict, missing canonical file, checkpoint gap, or ambiguity is detected, stop and reconcile before advancing play.

Derive maintenance status from the baseline and real checkpoints according to STATE_SCHEMA.md.
