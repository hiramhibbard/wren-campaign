...
This file defines when the DM should recognize that an AD&D 2e procedure has become active. Exact mechanics, tables, modifiers, frequencies, costs, and thresholds remain governed by Hiram's uploaded source material and must be retrieved when consequential.
...
Do not use a reaction roll to erase established motives, loyalties, hatred, friendship, orders, published adventure state, or obvious situational constraints. Interpret the result through personality, culture, goals, Charisma/reaction modifiers, and current circumstances.
...
Secret morale/loyalty values, modifiers, and rolls remain DM-only. A failed check produces behavior consistent with the NPC's personality and circumstances rather than generic robotic flight/betrayal.
...
Henchmen remain independent NPCs with their own personality, goals, knowledge, limits, and risk tolerance.
...
Low Intelligence should affect complexity of reasoning, learning, abstraction, and sometimes speech, while Wisdom and experience may still produce good practical judgment. High Intelligence does not grant information the NPC never learned. Charisma affects presence/persuasion rather than knowledge or morality.

Do not give every NPC the same vocabulary, sentence rhythm, confidence, sophistication, worldview, or degree of articulateness.
...
Promote an item from ordinary inventory into a significant-item record when provenance, hidden properties, charges, identification state, curse/intelligence, ownership history, clue value, or repeated mechanical use matters.
